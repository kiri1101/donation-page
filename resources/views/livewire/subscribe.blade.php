<div>
    ...
    <form wire:submit.prevent="submit">   
        <div class="flex flex-row gap-4 mt-2 md:mt-6">
            ...
            <button type="submit" class="mt-2"><span class="text-black font-semibold border-b-2 border-transparent transition-colors duration-200 hover:text-yellow-400 transform border-yellow-300">{{ __('Subscribe') }}</span></button>
        </div>
        @error('email') <span class="text-xs text-red-500 mt-1">{{ $message }}</span>@enderror
    </form>
    @if ($success)
    <div x-data="{ showAlert: true }">
        <div x-cloak x-show="showAlert" x-init="setTimeout(() => showAlert = false, 5000)" role="alert">
